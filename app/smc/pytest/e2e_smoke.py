...
def power_state_toggle_test(arc_chip_dut, asic_id, board_name):
    ...
    Also verify that PCIe gen downgrades to 1 when power state is set to low, maximum
    supported by system when power state is set to high.
    """
    expected_power_delta = 80
    settling_time = 0.5
    arc_chip = pyluwen.detect_chips()[asic_id]
    skip_tdp = _skip_boards(board_name)
    pcie_max_gen = 4 if board_name in ["quietbox2"] else 5
    ...
    # Verify PCIe gen is maximum supported by system
    assert pcie_gen == pcie_max_gen, (
        f"PCIE generation ({pcie_gen}) is not {pcie_max_gen}"
    )
